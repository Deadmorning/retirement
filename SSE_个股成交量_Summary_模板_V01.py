...
### 待补充case函数反馈抓取异常；
DDD_sz000651 = ak.stock_zh_a_tick_tx_js(code=company_code)


# query data is all string of the DataFrame formate
# pre handle to numeric and datatime and set datetimeindex
rowdata = DDD_sz000651.apply(pd.to_numeric,errors= 'ignore')
...
rowdata_datetime = DDD_sz000651.apply(pd.to_numeric,errors= 'ignore')
...
from sqlalchemy import create_engine
conn_sz000651 = create_engine('mysql+pymysql://root:cucumber@localhost/sz000651',encoding='utf-8')

rowdata_datetime.to_sql(name='sz000651_DDD_rowdata', con=conn_sz000651, if_exists='append', index=False)

reviewdata.to_sql(name='sz000651_DDD_reviewdata', con=conn_sz000651, if_exists='append', index=False)